...
The gender pay gap has always been a relevant issue for women seeking equal workplace rights. As will be mentioned later, our data only spans from 2013-2016, yet gender pay discrepancies are nothing new. The gender pay gap narrowed a mere 4% from 2003 to 2024 generally, with women earning 19% less than men in 2003 and 15% less in 2024 (Fry & Aragão, 2025). The closing of this wage gap is slow-moving, and strides towards resolution and full equality do not seem close. Some studies estimate it taking anywhere between 63 to 134 years for women to earn as much as men for the same job (Holder 2025, The Gender). Not only are these discrepencies unfair in the short term, but they also impact working women post-retirement: when paid less, women end up receiving less in Social Security and pensions and up to 30% less than men in retirement funds for the same job and number of years worked (The Simple). 
With the government’s recent changes to DEI policies, understanding workplace wage equality (or lack thereof) from a statistical perspective has never been more crucial to making strides in a progressive, educated direction. Thus, our research aims to explore how gender pay gaps vary by percentages of female representation in the workforce, and by specific major job categories (such as the Management, Business, and Financial sector).
...
We will primarily consider the variables of female wages as a percent of male wages (`wage_percent_of_male`), major job category (`major_category`), and percentage of females in the workforce (`percent_female`). To clarify-- if women make 90 cents for every 1 dollar men make doing the same job, then `wage_percent_of_female` will equal 0.90.
...
Figure 3 displays the distribution of female wages as a percent of male wages across all workforces observed. The unimodal, left-skewed distribution is concentrated below the dashed line, which indicates the ideal 1:1 ratio of female to male wages. The median female wage as a percent of male wage is 85.2% with a standard deviation of 9.4%.
...
We hypothesize that greater representation of females in the workforce will be correlated with higher female wages as a percent of male wages independent of and across all job categories. More specifically, we predict the job categories with the highest proportion of observations that were male-majority workforces (see Figure 2), including the categories of "Computer, Engineering, and Science", "Natural Resources, Construction, and Maintenance", and "Production, Transportation, and Material Moving", will have lower female wages as a percent of male wages.

## Methodology

We chose to do an exploratory analysis first to better understand the relationships between variables. Figure 1 displays the distributions of female wages as a percent of male wages across major job categories, suggesting the type of job could be a useful predictor of wage gaps due to the great variation in median and 25th to 75th quartiles. Figure 2 highlighted our job categories of interest for our hypotheses: "Computer, Engineering, and Science", "Natural Resources, Construction, and Maintenance", and "Production, Transportation, and Material Moving". Figure 3 displays our response variable of interest.

We chose to create three models to predict female wages as a percentage of male wages based on `major_category` and `percent_female`. We chose to do linear models because our response variable is numerical and we have numerical and categorical predictor variables. As we will discuss, Figure 4 demonstrates that a linear model is appropriate due to the moderately strong, positive, and linear relationship between our response variable and the percent of females in the workforce. 
...
Our third model is an interaction model between the percentage of the workforce that is female and the major job category. Scatterplots visualize these relationships and the fitted lines.
...
>>>>>>> b500faca0588894f7529a25ff873c8b61a932c77

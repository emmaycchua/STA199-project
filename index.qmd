...
Bureau of Labor through the U.S. Census.
...
med_earnings_gap by male_dom_workforce
Logistic regression model?
med_earnings_gap by perc_male_workforce colored by year
Scatterplot
...
ggplot(jobs_gender, aes(x = male_dom_workforce, fill = male_dom_earnings)) +
  ...
  labs(
    title = "Relationship between majority-male workforce and gender pay gap",
...
ggplot(jobs_gender_year, aes(x = workforce_gap_prop, y = wage_percent_of_male, color = year)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Relationship between gender differences in workforce and wage gaps",
    x = "Proportion of male workforce that is greater than the female workforce",
...
earnings_workforce_gaps_fit <- linear_reg() |>
  fit(wage_percent_of_male ~ workforce_gap_prop, data = jobs_gender)
tidy(earnings_workforce_gaps_fit)
glance(earnings_workforce_gaps_fit)
```

```{r}
#| label: earnings-workforce-logistic-reg
ggplot(jobs_gender, aes(x = male_dom_workforce, fill = male_dom_earnings)) +
  geom_bar(position = "dodge") +
  labs(
    title = "Relationship between majority-male workforce and gender pay gap",
...
workforce_log_fit <- logistic_reg() |>
	fit(male_dom_workforce ~ wage_percent_of_male, data = jobs_gender)
earnings_log_fit <- logistic_reg() |>
	fit(male_dom_earnings ~ workforce_gap_prop, data = jobs_gender)
tidy(earnings_log_fit)
tidy(workforce_log_fit)
```

...
Thus, our research aims to explore how gender pay gaps vary by percentages of female representation in the workforce, and by specific major job categories (such as the Management, Business, and Financial sector).
...
To clarify-- if women make 90 cents for every 1 dollar men make doing the same job, then `wage_percent_of_female` will equal 0.90.
...
Figure 3 displays our response variable of interest.
...
We did not choose to use `year` as a variable because Table 1 did not indicate that it could be a promising predictor.

We then selected the model with the highest $Adjusted~R^2$ and performed a hypothesis test on it using 1000 reps.
...
The visual from this model helps visualize wage differences in male versus female dominated fields because each line has a different slope, with some going in the negative direction and others in the positive.
...
For our three male-majority job categories of interest, we have written the simplified equations below.
...
The p-value is 0, which means that we should reject the null hypothesis; based off our analysis, assuming the null hypothesis is true, the probability of observing what we did is 0.
...
Some examples of potential issues related to the reliability and validity of our data include- but are not limited to-  recall bias, sampling error, changes in survey design, and coding bias. With data being from the Census Bureau and the Bureau of Labor, responses were likely recorded using a survey. Surveys can lead to multiple issues with reliability: for one, individuals who participated may experience recall bias, mis-reporting their income or other factors of data. Further, not *all* working American people were included in this dataset, so while it may be generalizable, there is a chance for sampling error/a misrepresentation of the wider population from this dataset. Overall, these impacts on reliability and validity are possible and unavoidable in the data we used for our analysis.
...
=======
From the p-values we obtained from our hypothesis testing and using a threshold value of 0.05, we reject the null hypothesis that female wage as a percent of male wage is independent of both the percent of females in the workforce, the major job category, and the interaction between these predictor variables. By rejecting the null, we are concluding that— while not necessarily a causal relationship– there *is* a relationship between percent of females in the workforce, the major job category, and female wages as a percent of male wages. This relationship varies by job category.
	
While we attempted to minimize limitations in our data and analysis, some were unavoidable. For one, the time period of data collection that we used in our analysis is between 9 and 13 years old. As discussed in the introduction, pay gaps have not historically changed much over the years, but it could nonetheless be beneficial for the data used in our analysis to be within the last 10 years for the most accurate, up-to-date analysis possible. Another limitation of our data is the fact that only full-time jobs were reported; this leaves out a possibly interesting area of study including part-time job pay gap trends. Finally, as discussed in our introduction, some data points have “N/A” values for the `wage_percent_male` variable, which may have limited certain analyses– however, with the number of these being so low in comparison to the total number of observations, this limitation should not largely impact the outcomes of our analysis.

To improve our data, we think it could be beneficial to do a more in-depth exploratory data analysis to determine which transformations could make the data best fit a linear model. We also could improve our data by estimating “N/A” values instead of completely ignoring them to help us reach the most accurate conclusions.
	
Some examples of potential issues related to the reliability and validity of our data include- but are not limited to-  recall bias, sampling error, changes in survey design, and coding bias. With data being from the Census Bureau and the Bureau of Labor, responses were likely recorded using a survey. Surveys can lead to multiple issues with reliability: for one, individuals who participated may experience recall bias, mis-reporting their income or other factors of data. Further, not *all* working American people were included in this dataset, so while it may be generalizable, there is a chance for sampling error/a misrepresentation of the wider population from this dataset. Changes in survey design from 2013-2016 could also lead to varying replies from respondents, which also impacts the reliability of the data. Overall, these impacts on reliability and validity are possible and unavoidable in the data we used for our analysis.
	
In the future, we think analyzing the effects of confounding factors such as race, age, and whether or not individuals were full- or part-time workers could be interesting. Gender may not be the only variable with discrepancies in pay, and it would be interesting to see how other demographic data impacts results. Further, we also think analyzing a more longitudinal dataset (i.e., from 2013 to present, 2025) could be intriguing in seeing how the gender pay gap has changed over time, if at all.
>>>>>>> 7e11afbe48399bcdc8d4f3f9c18a2c7c0fec7a08
=======
From the p-values we obtained from our hypothesis testing and using a threshold value of 0.05, we reject the null hypothesis that female wage as a percent of male wage is independent of both the percent of females in the workforce, the major job category, and the interaction between these predictor variables.
By rejecting the null, we are concluding that— while not necessarily a causal relationship– there *is* a relationship between percent of females in the workforce, the major job category, and female wages as a percent of male wages.
This relationship varies by job category.

While we attempted to minimize limitations in our data and analysis, some were unavoidable.
For one, the time period of data collection that we used in our analysis is between 9 and 13 years old.
As discussed in the introduction, pay gaps have not historically changed much over the years, but it could nonetheless be beneficial for the data used in our analysis to be within the last 10 years for the most accurate, up-to-date analysis possible.
Another limitation of our data is the fact that only full-time jobs were reported; this leaves out a possibly interesting area of study including part-time job pay gap trends.
Finally, as discussed in our introduction, some data points have “N/A” values for the `wage_percent_male` variable, which may have limited certain analyses– however, with the number of these being so low in comparison to the total number of observations, this limitation should not largely impact the outcomes of our analysis.
>>>>>>> 3516905c4550f6aae4ace89e9a4f9606991085c8

To improve our data, we think it could be beneficial to do a more in-depth exploratory data analysis to determine which transformations could make the data best fit a linear model.
We also could improve our data by estimating “N/A” values instead of completely ignoring them to help us reach the most accurate conclusions.

Some examples of potential issues related to the reliability and validity of our data include- but are not limited to: recall bias, sampling error, changes in survey design, and coding bias.
With data being from the Census Bureau and the Bureau of Labor, responses were likely recorded using a survey.
Surveys can lead to multiple issues with reliability: for one, individuals who participated may experience recall bias, misreporting their income or other factors of data.
Further, not *all* working American people were included in this dataset, so while it may be generalizable, there is a chance for sampling error/a misrepresentation of the wider population from this dataset.
Changes in survey design from 2013-2016 could also lead to varying replies from respondents, which also impacts the reliability of the data.
Overall, these impacts on reliability and validity are possible and unavoidable in the data we used for our analysis.

In the future, we think analyzing the effects of confounding factors such as race, age, and whether or not individuals were full- or part-time workers could be interesting.
Gender may not be the only variable with discrepancies in pay, and it would be interesting to see how other demographic data impacts results.
Further, we also think analyzing a more longitudinal dataset (i.e., from 2013 to present, 2025) could be intriguing in seeing how the gender pay gap has changed over time, if at all.

...
The gender pay gap has always been a relevant issue for women seeking equal workplace rights. Given the current political state and changes to DEI policies, this topic is crucial to understand and address from a statistical perspective. We are interested in how gender pay gaps vary across time from 2013 to 2016, varying percentages of female representation in the workforce, and specific major job categories, such as the Management, Business, and Financial sector.

We hypothesize that major job categories that tend to be “male-dominated” will have a more significant male-female pay gap and fewer women working in that career. Our research question asks, to what extent does the gender pay gap vary between male-dominated and female-dominated workforces and across major job categories?
...
We also learned that computer, engineering, and scinece professions have the least versatility in wage inequality.

\*\*\*\*\*\*\*ASK DOM ABOUT HOW TO FLIP IT AGAIN TO KEEP IT CONSISTENT\*\*\*\*\*\*\*
...
Male-dominated fields will be specified through the creation of two binary variables: `male_dom_workforce` labels observations with "Male majority" when the number of male workers is greater than the number of female workers and "Female majority" when there are more female workers and `male_dom_earnings` labels observations with "Yes" when the total male earnings are greater than the total female earnings and "No" when there are higher female earnings. The following table shows the pattern that regardless of year, the amount of occupations showing male dominated earnings in this study remains around 300, and that occupations with female dominated earnings, while showing a slight increase over time, are still under 11. Therefore, we do not put emphasis on year in this study as numbers remain fairly consistent.
...
Figure 2 shows the proportion of occupations within the different major categories that reported having a male or female dominated work force. The dashed line shows the 50% mark in the proportions, so any bar showing the blue male-majority line surpassing the dashed line has majority male-dominated work forces. The graph shows that there is the least amount of female representation across observations in the job categories of natural resources, construction, and maintenance category making it a male dominated field. However, the careers in sales/office, healthcare, and education are female-dominated.
...
\*\*\*\*\*\*\*\*\* ASK DOM IF WE NEED THIS VISUALIZATION TOO AND GENERALLY WHAT ELSE WE SHOULD INCLULDE FOR EDA\*\*\*\*\*\*\*

The final visualization displays that most women earn under 90% of a man's wage.
...
jobs_gender |>
  ...
  labs(
    title = "Figure 3: 
Distribution of the percentage of male wages that females earn",
    x = "Percent of male wages"
  ) +
  theme_minimal()
```

...
Based on the following visualization, we observed that all observations where women are paid equally or more than men are outside of the 75th percentile of observations, and are often considered outliers. We also learned that computer, engineering, and scinece professions have the least versatility in wage inequality.
...
Figure 2 shows __________. There is the least amount of female representation across observations in the job categories of natural resources, construction, and maintenance category making it a male dominated field. However, the careers in sales/office, healthcare, and education are female-dominated.
...
ggplot(jobs_gender, aes(x = major_category, fill = male_dom_workforce)) +
  ...
  labs(
    title = "Figure 2: Male-dominated workforces based on job categories",
...
jobs_gender |>
  ...
  geom_histogram() +
  labs(
    title = "Figure 3: Distribution of the percentage of male wages that females earn",
...
We hypothesize that greater representation of females in the workforce will be correlated with higher female wages as a percent of male wages independent of and across all job categories. We also predict the job categories with the highest proportion of observations that were male-majority workforces (see Figure 2), including the categories of "Computer, Engineering, and Science", "Management, Business, and Financial", and "Production, Transportation, and Material Moving", will have lower female wages as a percent of male wages.
...
ggplot(
  ...
  labs(
    title = "Figure 5: Additive model of gender differences in workforce and wage 
...
Holding all other variables constant, for every additional percentage point of female employees in the workforce, we expect the female wage as a percent of male wage to be higher, on average, by 5.3 percentage points. The intercept adjustments for non-baseline major job categories are given above. Holding all other variables constant, we expect an observation within the major job category of Education, Legal, Community Service, Arts, and Media to have a female wage as a percent of male wage that is lower, on average, by 2.0 percentage points.
...
ggplot(jobs_gender, aes(
  ...
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Figure 6: Interaction model of gender differences in workforce and wage 
gaps by job category",
    x = "Percent of workforce that is female",
    y = "Female wages as percent of male wages",
    color = "Major job category"
...
&\quad +0.222 \times percent~female~workforce \times Education,~Legal,~Community~Service,~Arts,~and~Media \\
&\quad +0.403 \times percent~female~workforce \times Healthcare~Practitioners~and~Technical \\
&\quad +0.079 \times percent~female~workforce \times Management,~Business,~and~Financial \\
&\quad +0.075 \times percent~female~workforce \times Natural~Resources,~Construction,~and~Maintenance \\
&\quad +0.128 \times percent~female~workforce \times Production,~Transportation,~and~Material~Moving \\
&\quad +0.302 \times percent~female~workforce \times Sales~and~Office \\
&\quad +0.171 \times percent~female~workforce \times Service \\
\end{aligned}
$$
Based on the linear model above, we predict that female employee wages as a percent of male wages when there are no female workers and the job category is Computer, Engineering, and Science will be 90.9%, on average. This makes sense given the well-known disparities in women's wages in STEM fields.
Holding all other variables constant, for every additional percentage point of female employees in a Computer, Engineering, and Science category workforce, we expect the female wage as a percent of male wage to be lower, on average, by 13.8 percentage points. This intercept is inconsistent with our prediction that workforces with higher female representation will have higher female wages as a percent of male wages. The intercept and slope adjustments for non-baseline major job categories are given above. The following is an example of a simplified equation if the major job category is Education, Legal, Community Service, Arts, and Media:
$$\widehat{wage~percent~of~male} = (0.909 - 0.094) + (-0.138 + 0.222) \times percent~female~workforce$$
$$\widehat{wage~percent~of~male} = 0.815 + 0.084 \times percent~female~workforce$$
Holding all other variables constant, we expect an observation within the major job category of Education, Legal, Community Service, Arts, and Media to have a female wage as a percent of male wage that is higher, on average, by 8.4 percentage points. This finding is consistent with our idea that a female-stereotyped field like education would have higher female wages as a percent of male wages.
